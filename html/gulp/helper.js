...
let dev = (argv && argv.dev !== undefined) ? argv.dev : (config && config.dev !== undefined ? config.dev : false);
let directory = (argv && argv.output !== undefined) ? argv.output : (config && config.output !== undefined ? config.output : 'dist');
...
module.exports.uglify = function(paths, directory) {
  return gulp.src(paths)
    ...
    .pipe(gulp.dest(`./${directory}/assets/js/creatiixDesign-script`));
}
